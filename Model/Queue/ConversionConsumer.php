 ...
 *
 * Processes conversion tracking messages from the queue with retry logic.
 *
...
/**
 * Consumer for processing conversion tracking messages
 */
class ConversionConsumer
{
    ...
    /**
     * Process conversion tracking message with retry logic
     *
        ...
        while ($attempt < Constants::MAX_RETRIES) {
            ...
            try {
                ...
                if ($statusCode >= 400 && $statusCode < 500 && $statusCode !== 429) {
                    $this->logger->error('Conversion tracking failed with client error', [
                ...
                $lastException = $e;
                $this->logger->debug('Conversion tracking attempt failed', [
                    'attempt' => $attempt,
                    'order_id' => $orderId,
                    'error' => $e->getMessage()
                ]);
